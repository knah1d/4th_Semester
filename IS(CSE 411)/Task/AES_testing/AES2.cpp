...
unsigned char * g(unsigned char wInput[4], int counter) {
    unsigned char * wReady = new unsigned char[4];
...
{
    unsigned char * tempC = new unsigned char[16];
...
void AESEncryptMultiple(unsigned char *plaintext, int plaintextLength, unsigned char *expandedKey, unsigned char *ciphertext) {
    int numBlocks = plaintextLength / 16; // Number of 16-byte blocks
    int remainder = plaintextLength % 16; // Remaining bytes

    // Encrypt full blocks
    for (int i = 0; i < numBlocks; ++i) {
        AESEncryption(plaintext + i * 16, expandedKey, ciphertext + i * 16);
    }

    // Encrypt remaining bytes (if any)
    if (remainder > 0) {
        // Pad remaining bytes if needed
        unsigned char paddedPlaintext[16];
        memcpy(paddedPlaintext, plaintext + numBlocks * 16, remainder);
        memset(paddedPlaintext + remainder, '%', 16 - remainder);
        for(int i=0; i<16; i++) {
            printf("%02x", paddedPlaintext[i]);
        }
        // Perform encryption
        AESEncryption(paddedPlaintext, expandedKey, ciphertext + numBlocks * 16);
    }
}

void AESDecryptMultiple(unsigned char *ciphertext, int ciphertextLength, unsigned char *expandedKey, unsigned char *decryptedText) {
    int numBlocks = ciphertextLength / 16; 
    int remainder = ciphertextLength % 16; 

    for (int i = 0; i < numBlocks; ++i) {
        AESDecryption(ciphertext + i * 16, expandedKey, decryptedText + i * 16);
    }

    if (remainder > 0) {

        unsigned char paddedCiphertext[16];
        memcpy(paddedCiphertext, ciphertext + numBlocks * 16, remainder);
        memset(paddedCiphertext + remainder, 0, 16 - remainder);

        
        AESDecryption(paddedCiphertext, expandedKey, decryptedText + numBlocks * 16);
    }
}


int main() {
    unsigned char plainText[200] = "shafiul alam khan is an MF";
    ...
    AESEncryptMultiple(plainText, size, expandedKey, cipher);
    ...
    cout << "\nEncrypted: 0x";
    output(cipher, size);
    //cout<<"Hello"<<sizeof(cipher);
    cout << endl;

    AESDecryptMultiple(cipher, size, expandedKey, decrypted);
    //AESDecryption(cipher, expandedKey, decrypted);
    cout << "\nDecrypted: 0x";
    output(decrypted, size);
    cout << endl;
    cout<<"Ascii: ";
    cout<<decrypted<<endl;
